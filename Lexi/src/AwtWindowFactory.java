 ...
 * Provides methods for instantiating AwtWindowimp objects.
 */

import java.awt.*;
import java.awt.event.*;

public class AwtWindowFactory extends WindowImpFactory {
	private static final AwtWindowFactory instance = new AwtWindowFactory();

	/**
	 * Static block which allows the class to register its instance with the parent class.
	 */
	static {
		WindowImpFactory.register("AwtWindow", instance);
	}
	...
	/**
	 *Pattern/Participant: Bridge(151): ConcreteimplementorB
	 * Class that models a specific Window implementation.
	 */
	private class AwtWindow implements WindowImp {
		...
		private class Pane extends Panel {
			private static final long serialVersionUID = -7230567547299085813L;
		...
		private AwtWindow(String title, Window window) {
			_window=window;
