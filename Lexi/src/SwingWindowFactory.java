 ...
 * Pattern/Participant: FactoryMethod(107): ConcreteCreator
 * Provides methods for instantiating SwingWindowImp objects.
 */
...
public class SwingWindowFactory extends WindowImpFactory {
	private static final SwingWindowFactory instance = new SwingWindowFactory();

	/**
	 * Static block which allows the class to register its instance with the parent
class.
	 */
	static {
		WindowImpFactory. register( "SwingWindow", instance);
	}
	...
	/**
	 * WindowImpFactory#newWindowImp(String, Window)
	 */
	public WindowImp newWindowImp(String title, Window w) {
		return new SwingWindow(title,w);
	}

	/**
	 * Pattern/Participant: Bridge(151): ConcreteimplementorA
	 * Class that models a SwingWindow implementation.
	 */
	private class SwingWindow implements WindowImp {
		...
		private class Pane extends JPanel {
			...
			private static final long serialVersionUID = -3513550975103944205L;
		...
		private SwingWindow(String title, Window window) {
			_window=window;
